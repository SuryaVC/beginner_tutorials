 ...
 * @brief Integration tests for ROS 2 publisher using GoogleTest framework. This file contains tests for verifying 
 *        the correct creation and functionality of a ROS 2 publisher within a custom node. It includes tests for 
 *        checking the number of publishers on a specific topic along with tf2 broadcaster and the successful operation 
 *        of ROS 2 functionalities.
 * @version 0.1
 * @date 2023-11-21
 *
...
class TaskTalker : public testing::Test {
 protected:
  /**
   * @brief Shared pointer to a ROS node, used for setting up a publisher or subscriber in tests.
   */
  rclcpp::Node::SharedPtr node_;
};

/**
 * @brief Tests if the publisher is able to publish a message on the 'chatter' topic.
 * 
 * Creates a publisher on the 'chatter' topic and a corresponding subscriber. It then checks if 
 * a message is successfully published and received by the subscriber.
 */
...
TEST_F(TaskTalker, test_tf2) {
  ...
  auto transform_available = tf_buffer->canTransform(tgt_frame, src_frame, now, 50ms);
