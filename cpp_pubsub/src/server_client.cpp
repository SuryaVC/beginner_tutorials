...
/**
 * @class ServerClient
 * @brief ServerClient class that creates a node to send a new string to the
 * server.
...
class ServerClient : public rclcpp::Node {
 public:
  ...
  ServerClient() : Node("server_client") {
    client =
        this->create_client<cpp_pubsub::srv::UpdateMessage>("service_node");
  }
  ...
  auto getRequest(char **argv) {
    auto request = std::make_shared<cpp_pubsub::srv::UpdateMessage::Request>();
